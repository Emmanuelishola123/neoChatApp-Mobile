...
const CallPreview = (props: callProps) => {
    const [statusBaseParams, setStatusBaseParams] = useState<{ color: string, icon: string } | undefined>(undefined)

    useEffect(() => {
        return () => {
            switch (props.status) {
                case 'sent':
                    setStatusBaseParams({ color: callStatusColor.SENT, icon: callStatusIconName.SENT, })
                    break;
                case 'received':
                    setStatusBaseParams({ color: callStatusColor.RECEIVED, icon: callStatusIconName.RECEIVED, })
                    break;
                case 'missed':
                    setStatusBaseParams({ color: callStatusColor.MISSED, icon: callStatusIconName.MISSED, })
                    break;
                case 'canceled':
                    setStatusBaseParams({ color: callStatusColor.CANCELED, icon: callStatusIconName.CANCELED, })
                    break;
                default:
                    return;
            }
