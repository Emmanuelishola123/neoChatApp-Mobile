...
const ChatFooter = (prop: ChatFooterProp) => {
    ...
    return (
        <View className='w-full h-fit space-x-1 p-2 flex-row justify-between items-center bg-[#f9f9f9]'>
            ...
            >
                {prop.messageInput.length > 0 ? <MaterialIcons name='send' size={24} color="#f9f9f9" /> : <MaterialIcons name="keyboard-voice" size={24} color="#f9f9f9" />}
            </Pressable>
        </View>
    )
}
