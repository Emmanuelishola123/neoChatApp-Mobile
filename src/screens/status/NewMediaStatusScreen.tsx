...
import { Image, Pressable, TextInput, View, screenHeight, screenWidth } from '../../components/styled';
import { useNavigation } from '@react-navigation/native';
import { FontAwesome5, FontAwesome, MaterialIcons } from '@expo/vector-icons';
import ExpoCamera from '../../components/Camera';
import { mediaType } from '../../components/Editor/type';
  ...
  return (
    <SafeAreaView style={{ backgroundColor: 'white', flex: 1, position: 'relative' }}>
      ...
      <View className='w-full flex-1 z-10'>
        {/* HEADER */}
        {media && <View className='flex-row items-start justify-between px-4 py-2 w-full'>
          <Pressable
          ...
          <View className=' flex-1 justify-center items-center mx-auto'>
            {
              media ?
                (<View style={{ width: screenWidth, height: screenHeight }}>
                  <Image source={{ uri: media }} resizeMode='contain' className='w-full h-full' />
                </View>)
        ...
        {/* FOOTER */}
        {media && <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} >
